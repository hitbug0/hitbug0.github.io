...
def make_post(post_template, input_file_path, output_dir):
    ...
    old_code_blocks, new_code_blocks = analyze_code_blocks(soup) # コードブロックの置き換え前後箇所特定
    old_section_codes, new_section_codes, sections = analyze_sections(soup) # セクションのid変更と左サイドバーにおけるリストの作成
